...
long long int getdis(const po&l, const po&r) {
	long long int dis = 0;
	for (int i = 0; i <(int)l.coors.size(); ++i) {
...
//直方体内にある点の数を求める。
	...
	void get_closest(const T& apo, long long int &ans) {//最近傍点を求める。
...
int main(){
	...
	int n;cin>>n;
	std::vector<po> points(n,po(2));
	for(int i=0;i<n;i++){
		points[i].index=i;
		cin>>points[i].coors[0]>>points[i].coors[1];
	}
